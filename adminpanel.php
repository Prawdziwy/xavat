...
try { $db->query('UPDATE z_monsters SET hide_creature = 0'); } catch(PDOException $error) {}
$main_content .= '<h3>All creatures are now VISIBLE!</h3>';
}
elseif($visibility == "hidden") {
try { $db->query('UPDATE z_monsters SET hide_creature = 1'); } catch(PDOException $error) {}
...
$show_list = "no";
try { $monster = $db->query('SELECT * FROM z_monsters WHERE name = '.$db->quote($monster_name).';')->fetch(); } catch(PDOException $error) {}
...
try { $db->query('UPDATE z_monsters SET gfx_name = '.$db->quote($new_link).' WHERE name = '.$db->quote($monster_name).';'); } catch(PDOException $error) {}
...
try { $db->query('UPDATE z_monsters SET hide_creature = 1 WHERE name = '.$db->quote(stripslashes($monster_to_hide)).';'); } catch(PDOException $error) {}
}
...
$main_content .= '<li>'.$monster_to_hide;
try { $db->query('UPDATE z_monsters SET hide_creature = 0 WHERE name = '.$db->quote(stripslashes($monster_to_hide)).';'); } catch(PDOException $error) {}
}
...
try { $monsters = $db->query('SELECT * FROM z_monsters WHERE hide_creature != 1 ORDER BY name'); } catch(PDOException $error) {}
...
try { $monsters = $db->query('SELECT * FROM z_monsters WHERE hide_creature != 0 ORDER BY name'); } catch(PDOException $error) {}
...
try { $db->query('UPDATE z_spells SET hide_spell = "0"'); } catch(PDOException $error) {}
$main_content .= 'All spells are now <b>visible</b>!';
}
elseif($_REQUEST['allspells'] == 'hidden') {
try { $db->query('UPDATE z_spells SET hide_spell = "1"'); } catch(PDOException $error) {}
...
try { $db->query('UPDATE z_spells SET hide_spell = 0 WHERE name = "'.$_REQUEST['spell_name'].'"'); } catch(PDOException $error) {}
$main_content .= "<b>'".$_REQUEST['spell_name']."'</b> is now VISIBLE!";
}
if($_REQUEST['visible'] == "no") {
try { $db->query('UPDATE z_spells SET hide_spell = "1" WHERE name = "'.$_REQUEST['spell_name'].'"'); } catch(PDOException $error) {}
$main_content .= "<b>'".$_REQUEST['spell_name']."'</b> is now HIDDEN!";
}
}
}
try { $spells = $db->query('SELECT * FROM z_spells ORDER BY name'); } catch(PDOException $error) {}
...
try { $db->query("DELETE FROM ".$db->tableName('z_monsters').";"); } catch(PDOException $error) {}
...
try { $db->query('INSERT INTO '.$db->tableName('z_monsters').' (hide_creature, name, mana, exp, health, speed_lvl, use_haste, voices, immunities, summonable, convinceable, race, gfx_name) VALUES (0, '.$db->quote($name).', '.$mana.', '.$exp.', '.$health.', '.$speed_lvl.', '.$use_haste.', '.$db->quote($voices_string).', '.$db->quote($immunities_string).', '.$flags['summonable'].', '.$flags['convinceable'].', '.$db->quote($race).', '.$db->quote($gfx_name).');'); } catch(PDOException $error) {}
$names_added[] = $name;
...
try { $db->query("DELETE FROM ".$db->tableName('z_spells').";"); } catch(PDOException $error) {}
$main_content .= '<h2>Reload spells.</h2>';
$main_content .= '<h2>All records deleted from table \'z_spells\' in database.</h2>';
foreach($vocation_name['0'] as $voc_id => $voc_name)
...
foreach($conjurelist as $spellname)
{
	...
	{
		$voc_str = '';
		foreach($vocation_name as $prom => $arr)
	...
	try { $db->query('INSERT INTO '.$db->tableName('z_spells').' (name, spell, spell_type, mana, lvl, mlvl, soul, pacc, vocations, conj_count, hide_spell) VALUES ('.$db->quote($name).', '.$db->quote($spell_txt).', \''.$type.'\', \''.$mana.'\', \''.$lvl.'\', \''.$mlvl.'\', \''.$soul.'\', \''.$pacc.'\', '.$db->quote($vocations_to_db).', \''.$count.'\', \''.$hide_spell.'\')'); } catch(PDOException $error) {}
...
$type = 'instant';
$count = 0;
try { $db->query('INSERT INTO z_spells (name, spell, spell_type, mana, lvl, mlvl, soul, pacc, vocations, conj_count, hide_spell) VALUES ('.$db->quote($name).', '.$db->quote($spell_txt).', \''.$type.'\', \''.$mana.'\', \''.$lvl.'\', \''.$mlvl.'\', \''.$soul.'\', \''.$pacc.'\', '.$db->quote($vocations_to_db).', \''.$count.'\', \''.$hide_spell.'\')'); } catch(PDOException $error) {}
$main_content .= "Added: ".$name."<br/>";
}
$main_content .= '<center><form action="?subtopic=adminpanel" METHOD=post><div class="BigButton" style="background-image:url('.$layout_name.'/images/buttons/sbutton.gif)" ><div onMouseOver="MouseOverBigButton(this);" onMouseOut="MouseOutBigButton(this);" ><div class="BigButtonOver" style="background-image:url('.$layout_name.'/images/buttons/sbutton_over.gif);" ></div><input class="ButtonText" type="image" name="Back" alt="Back" src="'.$layout_name.'/images/buttons/_sbutton_back.gif" ></div></div></form></center>';
}
