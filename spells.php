...
$spells = $db->query('SELECT * FROM z_spells WHERE hide_spell != 1 ORDER BY '.$orderby.', lvl');
...
else
{
	foreach($spells as $spell)
	{
		...
		foreach($spell_vocations as $voc)
		{
			...
			if(!empty($vocation_name[$voc_info[0]][$voc_info[1]]))
			{
				$vocs .= $vocation_name[$voc_info[0]][$voc_info[1]];
