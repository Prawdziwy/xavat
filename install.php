...
if($_REQUEST['page'] == 'step') 
{
	if($config['site']['install'] != "no") 
	{
		if($_REQUEST['server_conf'] == 'yes' || ($_REQUEST['step'] > 2 && $_REQUEST['step'] < 6)) 
		{
			...
			$dbitefile = $config['server']['sqliteDatabase'];
			$encryptionType = '';
			...
			elseif(strtolower($config['server']['sqlType']) == "sqlite")
			{
				...
				$link_to_sqlitedatabase = $config['site']['server_path'].$dbitefile;
				try
				{
		...
		if($step == '3') 
		{
			...
			echo 'Installer try to add new tables and columns to database.<br>';
				...
				{
					//if sqlite
					try { $db->query('ALTER TABLE accounts ADD "page_lastday" INTEGER(11) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "email_new" VARCHAR(255) NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "email_new_time" INTEGER(15) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "created" INTEGER(11) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "rlname" VARCHAR(255) NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "location" VARCHAR(255) NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "page_access" INTEGER(11) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "email_code" VARCHAR(255) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "next_email" INTEGER(11) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "premium_points" INTEGER(11) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE accounts ADD "flag" VARCHAR( 255 ) NOT NULL;'); } catch(PDOException $error) {}
					echo "Added columns to table <b>accounts</b>.<br/>";
					try { $db->query('ALTER TABLE guilds ADD "description" TEXT NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE guilds ADD "logo_gfx_name" VARCHAR(255) NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					echo "Added columns to table <b>guilds</b>.<br/>";
					try { $db->query('ALTER TABLE players ADD "created" INTEGER(11) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE players ADD "nick_verify" VARCHAR(5) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE players ADD "old_name" VARCHAR(255) NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE players ADD "hide_char" INTEGER(11) NOT NULL DEFAULT 0;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE players ADD "comment" TEXT NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					echo "Added columns to table <b>players</b>.<br/>";
					try { $db->query('CREATE TABLE "z_news_big" (
					...
					try { $db->query('CREATE TABLE "z_news_tickers" (
						"date" INTEGER NOT NULL,
					...
					try { $db->query('CREATE TABLE "z_spells" (
					...
					try { $db->query('CREATE TABLE "z_monsters" (
					...
					try { $db->query('CREATE TABLE "z_ots_comunication" (
					...
					try { $db->query('CREATE TABLE "z_shop_offer" (
						"id" int(11) NOT NULL auto_increment,
					...
					try { $db->query('CREATE TABLE "z_shop_history_item" (
						"id" int(11) NOT NULL auto_increment,
					...
					try { $db->query('CREATE TABLE "z_shop_history_pacc" (
						"id" int(11) NOT NULL auto_increment,
						"to_name" varchar(255) NOT NULL default 0,
						"to_account" int(11) NOT NULL default 0,
						"from_nick" varchar(255) NOT NULL,
					...
					try { $db->query('CREATE TABLE "zaypay_payment" (
					...
					try { $db->query('CREATE TABLE "z_shop_points_bought" (
				...
				{
					//if mysql
					try { $db->query("ALTER TABLE `accounts` ADD `premium_points` INT( 11 ) NOT NULL DEFAULT '0';"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `email_new` VARCHAR( 255 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `email_new_time` INT( 15 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `email_code` VARCHAR( 255 ) NOT NULL DEFAULT '0';"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `next_email` INT( 11 ) NOT NULL DEFAULT '0';"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `created` INT( 11 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `page_lastday` INT( 11 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `page_access` INT( 11 ) NOT NULL DEFAULT '0';"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `rlname` VARCHAR( 255 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `location` VARCHAR( 255 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `accounts` ADD `flag` VARCHAR( 255 ) NOT NULL;"); } catch(PDOException $error) {}
					echo "Added columns to table <b>accounts</b><br/>";
					try { $db->query('ALTER TABLE `guilds` ADD `description` TEXT NOT NULL;'); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE `guilds` ADD `logo_gfx_name` VARCHAR( 255 ) NOT NULL;'); } catch(PDOException $error) {}
					echo "Added columns to table <b>guilds</b>.<br/>";
					try { $db->query("ALTER TABLE `players` ADD `created` INT( 11 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `players` ADD `nick_verify` VARCHAR( 5 ) NOT NULL;"); } catch(PDOException $error) {}
					try { $db->query('ALTER TABLE `players` ADD `old_name` VARCHAR(255) NOT NULL DEFAULT "";'); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `players` ADD `hide_char` INT( 11 ) NOT NULL DEFAULT '0';"); } catch(PDOException $error) {}
					try { $db->query("ALTER TABLE `players` ADD `comment` TEXT NOT NULL;"); } catch(PDOException $error) {}
					echo "Added columns to table <b>players</b><br/>";
					try { $db->query("CREATE TABLE `z_news_big` (
					...
					try { $db->query("CREATE TABLE `z_referers` (
					...
					try { $db->query("CREATE TABLE z_tracker (
					...
					try { $db->query("CREATE TABLE IF NOT EXISTS `z_changelog` (
							`id` int(11) NOT NULL auto_increment,
		...
		if($step == '4') 
		{
			...
			$check_news_ticker = $db->query('SELECT * FROM z_news_tickers WHERE image_id = 1 AND author = 1 AND hide_ticker = 0 LIMIT 1 OFFSET 0')->fetch();
			if(!isset($check_news_ticker['author'])) 
			{
				$db->query('INSERT INTO z_news_tickers (date, author, image_id, text, hide_ticker) VALUES 
			...
			$check_news = $db->query('SELECT * FROM z_news_big WHERE author = "Gesior" LIMIT 1 OFFSET 0')->fetch();
			if(!isset($check_news['author'])) 
			{
				$db->query('INSERT INTO z_news_big (hide_news, date, author, author_id, image_id, topic_df, text_df) VALUES 
			...
			$check_voc_0 = $db->query('SELECT * FROM players WHERE name = "Rook Sample" LIMIT 1 OFFSET 0')->fetch();
			if(!isset($check_voc_0['name'])) 
			{
				$db->query('INSERT INTO players 	(name, world_id, group_id, account_id, level, vocation, health, healthmax, experience, lookbody, lookfeet, lookhead, looklegs, looktype, lookaddons, maglevel, mana, manamax, manaspent, soul, town_id, posx, posy, posz, conditions, cap, sex, lastlogin, lastip, save, skull, skulltime, rank_id, guildnick, lastlogout, blessings, balance, stamina, direction, loss_experience, loss_mana, loss_skills, loss_containers, loss_items, premend, online, marriage, promotion, deleted, description, comment, created, hide_char, nick_verify) VALUES 
			...
			$check_voc_1 = $db->query('SELECT * FROM players WHERE name = "Sorcerer Sample" LIMIT 1 OFFSET 0')->fetch();
			...
			$check_voc_2 = $db->query('SELECT * FROM players WHERE name = "Druid Sample" LIMIT 1 OFFSET 0')->fetch();
			...
			$check_voc_3 = $db->query('SELECT * FROM players WHERE name = "Paladin Sample" LIMIT 1 OFFSET 0')->fetch();
			...
			$check_voc_4 = $db->query('SELECT * FROM players WHERE name = "Knight Sample" LIMIT 1 OFFSET 0')->fetch();
