...
<div class="news-top"></div><div class="news-mid"><div class="title-news-buy">
					<b>Postacie do wykupienia</b><br /><br />
					Na naszym serwerze istnieje mozliwosc kupna nieuzywanej przez dluzszy czas postaci. Jest to idealna opcja dla osob, ktorzy nie maja za wiele czasu na zdobywanie doswiadczenia i chca zaczac grac na naszym serwerze ale przerazaja je zbyt wysokie levele postaci. <u>Postac kupowana jest z wszelkimi przedmiotami jakie posiada (w DEPO, czy tez na sobie) oraz z domkiem o ile posiada taki.</u> Po zakupie postaci, mozesz dokonac zmiany nicku w sms shopie.<br />
					Na liste ponizej trafiaja gracze, ktorzy nie logowali sie do gry przez ostatni miesiąc. Jezeli Twoja postac trafila na liste postaci do kupienia (ponizej), a chcialbys by z niej zostala sciagnieta - wystarczy, ze zalogujesz sie do gry! Pospiesz sie jednak gdyz w przypadku gdy ktos juz kupi Twoja postac, nie istnieje mozliwosc jej zwrotu!<br /><br />
					Aby kupic postac zaloguj sie na konto (musisz posiadac wymagana ilosc punktow w sms shopie), przy wybranej postaci kliknij "buy". Nastepnie wpisz account number na ktory dana postac ma byc kupiona.
		...
		$i = 0;
		$offset = (int)($page * 100);
		$players = $db->query("SELECT `id`, `name`, `level`, `vocation`, `promotion` FROM `players` WHERE `lastlogin` < UNIX_TIMESTAMP() - " . $minTime . " AND `level` >= " . $minLevel . " AND `id` > 5 AND `name` NOT IN ('Account Manager', 'Druid Sample', 'Sorcerer Sample', 'Knight Sample', 'Paladin Sample', 'Sprite', 'Bodzio Chelikopter', 'God Sprite', 'God Alex', 'Alex', 'Xavato', 'God Xavato') ORDER BY `level` DESC LIMIT 151 OFFSET " . $offset)->fetchAll();
