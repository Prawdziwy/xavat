<?php
	...
	{
		if($logged)
			$user_premium_points = $account_logged->getCustomField('premium_points_guild');
		else
		...
		function getItemByID($id)
		{
			$id = (int)$id;
			global $db;
			$data = $db->query("SELECT * FROM `z_shop_offer_guild` WHERE `id` = " . $db->quote($id) . ";")->fetch();
			...
			elseif($data['offer_type'] == 'addonitems')
			{
				$offer['id'] = $data['id'];
				$offer['type'] = $data['offer_type'];
				$offer['item_id'] = $data['itemid1'];
		...
		{
			global $db;
			$offer_list = $db->query("SELECT * FROM `z_shop_offer_guild` WHERE `hidden` = 0 ORDER BY `sort` ASC;")->fetchAll();
			$i_pacc = 0;
			$i_item = 0;
			$i_custom = 0;
			$i_dolls = 0;
			...
			foreach($offer_list as $data)
			{
				...
				elseif ($data['offer_type'] == 'custom')
				{
					$offer_array['custom'][$i_custom]['id'] = $data['id'];
					$offer_array['custom'][$i_custom]['item_id'] = $data['itemid1'];
					$offer_array['custom'][$i_custom]['item_count'] = $data['count1'];
					$offer_array['custom'][$i_custom]['points'] = $data['points'];
					$offer_array['custom'][$i_custom]['description'] = $data['offer_description'];
					$offer_array['custom'][$i_custom]['name'] = $data['offer_name'];
					$i_custom++;
				}
				elseif ($data['offer_type'] == 'dolls')
				{
					$offer_array['dolls'][$i_dolls]['id'] = $data['id'];
					$offer_array['dolls'][$i_dolls]['item_id'] = $data['itemid1'];
					$offer_array['dolls'][$i_dolls]['item_count'] = $data['count1'];
					$offer_array['dolls'][$i_dolls]['points'] = $data['points'];
					$offer_array['dolls'][$i_dolls]['description'] = $data['offer_description'];
					$offer_array['dolls'][$i_dolls]['name'] = $data['offer_name'];
					$i_dolls++;
				}
				elseif ($data['offer_type'] == 'addonitems')
				{
					$offer_array['addonitems'][$i_addonitems]['id'] = $data['id'];
					$offer_array['addonitems'][$i_addonitems]['item_id'] = $data['itemid1'];
					$offer_array['addonitems'][$i_addonitems]['item_count'] = $data['count1'];
					$offer_array['addonitems'][$i_addonitems]['points'] = $data['points'];
					$offer_array['addonitems'][$i_addonitems]['description'] = $data['offer_description'];
					$offer_array['addonitems'][$i_addonitems]['name'] = $data['offer_name'];
					$i_addonitems++;
				}
				elseif ($data['offer_type'] == 'container')
				{
		...
		if(empty($action))
		{
			...
			$query = $db->query("SELECT * FROM `z_shop_offer_guild` WHERE `offer_type` NOT LIKE 'container' AND `offer_type` NOT LIKE 'changename' ORDER BY `bought` DESC LIMIT 3"); 
				...
				<td bgcolor='.$bgcolor.'> 
					<center> 
						...
						if(!$logged)
							...
							$result .= '
							<form action="index.php?subtopic=guildshop&action=select_player" method=POST> 
			...
			$main_content .= '<div class="tabs">';
			$main_content .= '
			<ul class="tabNavigation">';
				if(count($offer_list['item']) > 0)
					$main_content .= '<li><a href="#first" id="tab1">-= Items =-</a></li>';
					
				if(count($offer_list['custom']) > 0)
					$main_content .= '<li><a href="#second" id="tab2">-= Custom Items =-</a></li>';	
				
				if(count($offer_list['dolls']) > 0)		
					$main_content .= '<li><a href="#third" id="tab3">-= Dolls =-</a></li>';
					
				if(count($offer_list['addonitems']) > 0)		
					$main_content .= '<li><a href="#fourth" id="tab4">-= Addon Items =-</a></li>';	
				...
				<div id="first">
					<table border="0" cellspacing="1" cellpadding="4" width="100%" id="iblue">
						<tr bgcolor="' . $config['site']['vdarkborder'] . '">
							<td colspan="3" style="font-weight: bold;">Items</td>
						</tr>
						<tr bgcolor="' . $config['site']['vdarkborder'] . '">
						...
						foreach($offer_list['item'] as $item)
						{
							...
							<tr bgcolor="' . $bgcolor . '">
								...
								<td align="center">' . (!$logged ? "Login to Buy" : "<form action=\"?subtopic=guildshop&action=select_player\" method=\"POST\"><input type=\"hidden\" name=\"buy_id\" value=\"{$item['id']}\"><input type=\"image\" src=\"{$layout_name}/images/buttons/buy.png\" value=\"Buy\"><br /><b>for {$item['points']} points</b></form>") . '</td>
							</tr>';
			...
			if(count($offer_list['custom']) > 0)
			{
				...
				<div id="second">
					<table border="0" cellspacing="1" cellpadding="4" width="100%" id="iblue">
						<tr bgcolor="' . $config['site']['vdarkborder'] . '">
							<td colspan="3" style="font-weight: bold;">Custom Items</td>
						</tr>
						<tr bgcolor="' . $config['site']['vdarkborder'] . '">
							<td width="50" align="center" style="font-weight: bold;">Picture</td>
							<td width="350" align="left" style="font-weight: bold;">Description</td>
						...
						foreach($offer_list['custom'] as $custom)
						{
							...
							$main_content .= '
							<tr bgcolor="' . $bgcolor . '">
								<td align="center"><img src="images/items/' . $custom['item_id'] . '.gif"></td>
								<td><strong>' . $custom['name'] . '</strong> (' . $custom['points'] . ' points)<br /><i>' . $custom['description'] . '</i></td>
								<td align="center">' . (!$logged ? "Login to Buy" : "<form action=\"?subtopic=guildshop&action=select_player\" method=\"POST\"><input type=\"hidden\" name=\"buy_id\" value=\"{$custom['id']}\"><input type=\"image\" src=\"{$layout_name}/images/buttons/buy.png\" value=\"Buy\"><br /><b>for {$custom['points']} points</b></form>") . '</td>
							</tr>';
			...
			if(count($offer_list['dolls']) > 0)
			{
				...
				<div id="third">
					<table border="0" cellspacing="1" cellpadding="4" width="100%" id="iblue">
						<tr bgcolor="' . $config['site']['vdarkborder'] . '">
							<td colspan="3" style="font-weight: bold;">Dolls</td>
						</tr>
						<tr bgcolor="' . $config['site']['vdarkborder'] . '">
							<td width="50" align="center" style="font-weight: bold;">Picture</td>
							<td width="350" align="left" style="font-weight: bold;">Description</td>
						...
						foreach($offer_list['dolls'] as $dolls)
						{
							$bgcolor = ($z % 2) ? $config['site']['darkborder'] : $config['site']['lightborder'];
							$main_content .= '
							<tr bgcolor="' . $bgcolor . '">
								<td align="center"><img src="images/items/' . $dolls['item_id'] . '.gif"></td>
								<td><strong>' . $dolls['name'] . '</strong> (' . $dolls['points'] . ' points)<br /><i>' . $dolls['description'] . '</i></td>
								<td align="center">' . (!$logged ? "Login to Buy" : "<form action=\"?subtopic=guildshop&action=select_player\" method=\"POST\"><input type=\"hidden\" name=\"buy_id\" value=\"{$dolls['id']}\"><input type=\"image\" src=\"{$layout_name}/images/buttons/buy.png\" value=\"Buy\"><br /><b>for {$dolls['points']} points</b></form>") . '</td>
							</tr>';
			...
			if(count($offer_list['addonitems']) > 0)
			{
				...
				<div id="fourth">
					<table border="0" cellspacing="1" cellpadding="4" width="100%" id="iblue">
						...
						foreach($offer_list['addonitems'] as $addonitems)
						{
							$bgcolor = ($z % 2) ? $config['site']['darkborder'] : $config['site']['lightborder'];
							$main_content .= '
							<tr bgcolor="' . $bgcolor . '">
								<td align="center"><img src="images/items/' . $addonitems['item_id'] . '.gif"></td>
								<td><strong>' . $addonitems['name'] . '</strong> (' . $addonitems['points'] . ' points)<br /><i>' . $addonitems['description'] . '</i></td>
								<td align="center">' . (!$logged ? "Login to Buy" : "<form action=\"?subtopic=guildshop&action=select_player\" method=\"POST\"><input type=\"hidden\" name=\"buy_id\" value=\"{$addonitems['id']}\"><input type=\"image\" src=\"{$layout_name}/images/buttons/buy.png\" value=\"Buy\"><br /><b>for {$addonitems['points']} points</b></form>") . '</td>
							</tr>';
				...
				<table border="0" cellspacing="1" cellpadding="4" width="100%" id="iblue">
					...
					foreach($offer_list['container'] as $container)
					{
						...
						<tr bgcolor="' . $bgcolor . '">
							...
							<td align="center">' . (!$logged ? "Login to Buy" : "<form action=\"?subtopic=guildshop&action=select_player\" method=\"POST\"><input type=\"hidden\" name=\"buy_id\" value=\"{$container['id']}\"><input type=\"submit\" value=\"Buy\"><br /><b>for {$container['points']} points</b></form>") . '</td>
						</tr>';
		...
		elseif($action == 'select_player')
		{
			...
			else
			{
				...
				if(empty($buy_id))
					$main_content .= 'Please <a href="?subtopic=guildshop">select item</a> first.';
				else
				{
					...
					if(isset($buy_offer['id']))
					{
						if($buy_offer['type'] != 'changename')
						{
							if($user_premium_points >= $buy_offer['points'])
							{
								...
								<form action="?subtopic=guildshop&action=confirm_transaction" method="POST">
							...
							{
								$main_content .= '<br />For this item you need <b>' . $buy_offer['points'] . '</b> points. You have only <b>' . $user_premium_points . '</b> premium points. Please <a href="?subtopic=guildshop">select other item</a> or buy premium points.';
							}
						}
					}
					else
						...
						if(isset($buy_offer['id']))
						{
							if($user_premium_points >= $buy_offer['points'])
							{
								if(check_name($buy_name))
								{
									...
									if($buy_player->isLoaded())
									{
										...
										if($_SESSION['viewed_confirmation_page'] == 'yes' && $_POST['buy_confirmed'] == 'yes')
										{
											if($buy_offer['type'] == 'pacc')
											{
												...
												$account_logged->setCustomField('premium_points_guild', $user_premium_points - $buy_offer['points']);
												$user_premium_points = $user_premium_points - $buy_offer['points'];
												
												if($player_premdays == 0)
													$buy_player_account->setCustomField('lastday', time());
													
												$main_content .= '<h2>PACC added!</h2><b>' . $buy_offer['days'] . ' days</b> of Premium Account added to account of player <b>' . $buy_player->getName() . '</b> for <b>' . $buy_offer['points'] . ' premium points</b> from your account.<br />Now you have <b>' . $user_premium_points . ' premium points</b>.<br /><a href="?subtopic=guildshop">Back</a>';
											}
											elseif($buy_offer['type'] == 'item' || $buy_offer['type'] == 'custom' || $buy_offer['type'] == 'dolls' || $buy_offer['type'] == 'addonitems')
											{
												...
												$main_content .= '<div id="tdno"><table><tr><td id="ired"><b>Item added!</b></td></tr><tr><td id="tdno"><b>'.$buy_offer['name'].'</b> added to player <b>'.$buy_player->getName().'</b> items (he will get this items after relog) for <b>'.$buy_offer['points'].' premium points</b> from your account.</td></tr><tr><td id="igreen">Now you have <b>'.$user_premium_points.' premium points</b>.<a href="?subtopic=guildshop">&nbsp;GO TO MAIN SHOP SITE</a></td></tr></table></div>';
											}
											elseif($buy_offer['type'] == 'container')
											{
												...
												$account_logged->setCustomField('premium_points_guild', $user_premium_points-$buy_offer['points']);
												$user_premium_points = $user_premium_points - $buy_offer['points'];
												$main_content .= '<div id="tdno"><table><tr><td id="ired"><b>Container of items has been added!</b></td></tr><tr><td id="tdno"><b>'.$buy_offer['name'].'</b> added to player <b>'.$buy_player->getName().'</b> items (he will get this items after relog) for <b>'.$buy_offer['points'].' premium points</b> from your account.</td></tr><tr><td id="igreen">Now you have <b>'.$user_premium_points.' premium points</b>.<a href="?subtopic=guildshop">&nbsp;GO TO MAIN SHOP SITE</a></td></tr></table></div>';
											}
										}
										else
										{
											...
											else
											{
												...
												<td align="right"><form action="?subtopic=guildshop" method="POST"><input type="submit" value="Cancel"></form></td></tr>
												</table>';
											}
										}
									}
									...
									{
										$main_content .= 'Player with name <b>'.$buy_name.'</b> doesn\'t exist. Please <a href="?subtopic=guildshop&action=select_player&buy_id='.$buy_id.'">select other name</a>.';
									}
								}
								else
								{
									$main_content .= 'Invalid name format. Please <a href="?subtopic=guildshop&action=select_player&buy_id='.$buy_id.'">select other name</a> or contact with administrator.';
								}
							}
							else
							{
								$main_content .= 'For this item you need <b>'.$buy_offer['points'].'</b> points. You have only <b>'.$user_premium_points.'</b> premium points. Please <a href="?subtopic=guildshop">select other item</a> or buy premium points.';
							}
						}
						else
						{
							$main_content .= 'Offer with ID <b>'.$buy_id.'</b> doesn\'t exist. Please <a href="?subtopic=guildshop">select item</a> again.';
						}
					}
				}
			}
