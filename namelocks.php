...
if($logged && $account_logged->getPageAccess() > 0)
{
	...
	$number_of_rows = 1;
	$players_info = $db->query("SELECT `players`.`name`, `players`.`old_name` AS `name_new` FROM `bans`, `players` WHERE `players`.`old_name` != '' AND `bans`.`value` = `players`.`id` AND `bans`.`active` = 1");
	$players = array();
